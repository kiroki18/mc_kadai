...
static std::unique_ptr<ExprAST> ParseIdentifierExpr() {
    // 1. getIdentifierを用いて識別子を取得する。Hiroi p.s.:識別子 => iden
    std::string iden = lexer.getIdentifier();
    // 2. トークンを次に進める。
    getNextToken();
    // 3. 次のトークンが'('の場合は関数呼び出し。そうでない場合は、
    // VariableExprASTを識別子を入れてインスタンス化し返す。
    if(CurTok != '(')
        //return std::unique_ptr<ExprAST> tmp_ptr = new std::unique_ptr<ExprAST>(VariableExprAST);
        return llvm::make_unique<VariableExprAST>(iden);
    // 4. '('を読んでトークンを次に進める。
    getNextToken();
    ...
    return llvm::make_unique<CallExprAST>(iden, std::move(args));
}
...
static std::unique_ptr<PrototypeAST> ParsePrototype() {
    ...
    getNextToken();
    // 7. CallExprASTを構成し、返す。
    return llvm::make_unique<PrototypeAST>(iden, std::move(args));
